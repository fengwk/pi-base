...
- For existing files, prefer `edit`; use `write` only for whole-file replacement or roughly 70%+ file-wide rewrites, never for localized edits that an `edit` can handle safely.
