...
- The report is returned to you but is NOT shown to the user; summarize it yourself. The result envelope includes the resumable id as `<task id="...">`.

Parameters:
- `subagent_type` (required)
- `description` (required): prefer a short 3-5 word task label shown in the UI
- `prompt` (required)
- `session_id` (optional)
