...
export function extractJdtUri(target: string): string | null {
...
export function formatWorkspaceSymbols(result: any, limit: number): string {
...
export function requireNonNegativeInteger(value: unknown, name: string, defaultValue: number): number {
...
export function getToolCwd(params: any, ctx: any): string {
  return resolveToolWorkdir(params.workdir, ctx.cwd ?? process.cwd()).cwd;
}

export function getResolverForPath(toolPath: string, cwd: string, resolverFactory?: LspResolverFactory): LspDiscoveryResolver {
  if (resolverFactory) return resolverFactory(dirname(resolveFromCwd(toolPath, cwd)));
  return new LspDiscoveryResolver({});
}

export function renderLspResult(toolName: string, result: any, renderOptions: any, theme: any, context: any, options: { getCollapsedResultLines?: CollapsedResultLinesResolver; getCollapsedResultMaxChars?: CollapsedResultMaxCharsResolver }): any {
  const collapsedLines = resolveCollapsedResultLines(toolName, undefined, context, options.getCollapsedResultLines);
  const maxCollapsedChars = resolveCollapsedResultMaxChars(toolName, undefined, context, options.getCollapsedResultMaxChars);
...
export async function executeLspWorkspaceSymbols(params: any, signal: AbortSignal | undefined, ctx: any, resolverFactory: LspResolverFactory | undefined, manager: { getClient: typeof import("./client.js").lspManager.getClient }): Promise<any> {
  try {
    throwIfAborted(signal);
    const cwd = getToolCwd(params, ctx);
    const filePath = resolveFromCwd(params.path, cwd);
...
export async function executeLspJavaDecompile(params: any, signal: AbortSignal | undefined, ctx: any, resolverFactory: LspResolverFactory | undefined, manager: { getClient: typeof import("./client.js").lspManager.getClient }): Promise<any> {
  try {
    throwIfAborted(signal);
    const cwd = getToolCwd(params, ctx);
    const workspaceFile = resolveFromCwd(params.path, cwd);
    const resolver = getResolverForPath(params.path, cwd, resolverFactory);
    const client = await withAbort(manager.getClient(workspaceFile, resolver), signal);
