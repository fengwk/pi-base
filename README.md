...
- `bash`, `read`, `edit`, `write`, `grep`, `find`, and `lsp_*` default `workdir` to the current working directory. If `workdir` is provided, path resolution or command execution uses that directory; for `bash`, prefer `workdir` over embedding `cd ... &&` in `command`.
